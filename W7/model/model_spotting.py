...
class RGBClassifier(nn.Module):
        ...
        def __init__(self, args = None):
            ...
            self._offset_head = nn.Linear(self._d, 1)  # [B, T, 1]
        ...
        def forward(self, x):
            ...
            if self._temporal_stride > 1:
                im_feat = self.temporal_downsample(im_feat, self._temporal_stride)
            ...
            if self._temporal_model == 'transformer':
                ...
                new_len = im_feat.shape[1]
                pos_enc = self._positional_encoding[:, :new_len, :]
                
                #im_feat_pos = im_feat + self._positional_encoding[:, :im_feat.size(1)].to(im_feat.device)
